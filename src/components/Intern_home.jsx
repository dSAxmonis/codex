        ...
        {isLoading ? (
          ...
          <main style={{ maxWidth: 1100, margin: "0 auto", padding: "40px clamp(14px,4vw,44px) 80px" }}>
            ...
            {posts.length === 0 ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(310px,1fr))", gap: 16 }}>
                {posts.map((post, i) => (
                  <CompanyCard key={post._id} post={post} delay={i * 0.04} openId={openId} setOpenId={setOpenId} />
                ))}
              </div>
            )}
          </main>
...
function CompanyCard({ post, delay, openId, setOpenId }) {
  const open = openId === (post._id || post.id);
  const setOpen = (val) => setOpenId(val ? (post._id || post.id) : null);
